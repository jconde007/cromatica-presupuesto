...
export default function App({ session, onSignOut }) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setArrastres(arr)
      // Ordenar cuentas según CUENTAS_DEFAULT (mismo orden cada mes)
      const orden = Object.fromEntries(CUENTAS_DEFAULT.map((c, i) => [c.nombre, i]))
      const ctsOrdenadas = [...cts].sort((a, b) => (orden[a.nombre] ?? 99) - (orden[b.nombre] ?? 99))
      setCuentasState(ctsOrdenadas)
      setAsignados(asig)
      setClabeMap(clMap)
      setDeadlines(dlns)
      setApartados(aps)
